...
def run_prompts(
    ...
    for prompt_batch in tqdm(dataloader, total=len(dataloader), desc="Running prompts"):
        ...
        # print(f'get_probs_kwargs: {get_probs_kwargs}')
        # print(f'get_probs: {get_probs}')
    ...
    for i, prompt in enumerate(prompts):
        # print(f' prompt.target_tokens: {prompt.target_tokens}')
...
def run_prompts_int(
    ...
    for prompt_batch in tqdm(dataloader, total=len(dataloader), desc="Running prompts"):
        ...
        probs.append(get_probs(nn_model, prompt_batch, scan=scan, **get_probs_kwargs))
        # print(f'probs[0].shape: {probs[0].shape}')
        # print(f'probs: {probs}')
        scan = False  # Not sure if this is a good idea
    probs = th.cat(probs)
    target_probs = []
