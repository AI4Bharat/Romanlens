...
class Prompt:
    ...
    def has_no_collisions(self, ignore_langs: Optional[str | list[str]] = None):
        ...
        for lang, lang_tokens in self.latent_tokens.items():
            # print(f'lang: {lang}')
            # print(f' tokens: {lang_tokens}')
            if lang in ignore_langs:
                continue
            tokens += lang_tokens
        # print(f'no collisions tokens : {tokens}')
        # print(f'ignore lang: {ignore_langs}')
...
lang2name = {
    ...
    "te": "తెలుగు",
    "hi_translit": "Hindi",
    "ml_translit": "Malayalam",
    "gu_translit": "Gujarati",
    "ta_translit": "Tamil",
    "te_translit": "Telugu",
    "ml_devnagari": "Malayalam_dng",
    "gu_devnagari": "Gujarati_dng",
    "ta_devnagari": "Tamil_dng",
    "te_devnagari": "Telugu_dng",
    "A": "A",
    "B": "B",
}
...
def get_unshifted_prompt_pairs(
    source_df,
    target_df,
    _source_prompts,
    _target_prompts,
    ...
    source_prompts = []
    target_prompts = []
      # Replace product with direct matching
    # print(f'source_df: {source_df.head}')
    # print(f'target_df: {target_df.head}')

    source_target = list(product(source_df.iterrows(), target_df.iterrows()))
    # matching_words = pd.merge(
    #     source_df, 
    #     target_df, 
    #     on='word_original',  # Match on same words
    #     suffixes=('_source', '_target')
    # )
    
    # print(f'matching_words: {matching_words}')
    # shuffle(source_target)
    selected_source_rows = set()
    selected_target_rows = set()
    # print(f'source_target: {source_target}')
    for (i, source_row), (j, target_row) in source_target:
        # print(f'source_row["word_original"]: {source_row["word_original"]}')
        # print(f'target_row["word_original"]: {target_row["word_original"]}')
        if source_row["word_original"] == target_row["word_original"]:
            print('yu')
    ...
    # print( f'source_prompts: {source_prompts}')
