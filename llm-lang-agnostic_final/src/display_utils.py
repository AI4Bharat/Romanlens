...
def plot_results_unshifted(
    ...
    colors = sns.color_palette("tab10", 2 ) #+ len(latent_probs)
...
def plot_results_alternative(
    ...
    source_baseline=None,
    target_baseline=None,
    method="quantiles"  # or "violin", "quantiles", "heatmap"
):
    colors = sns.color_palette("tab10", 1 + len(latent_probs))
    ...
    if method == "boxplot":
        ...
        positions = np.arange(target_probs.shape[1])  # layer positions
        ax.boxplot([target_probs[:, i] for i in range(target_probs.shape[1])],
                  positions=positions,
                  labels=[f"L{i}" for i in positions],
                  patch_artist=True,
                  boxprops=dict(facecolor=colors[0], alpha=0.5))
        
        # Plot latent probs
        for idx, (label, probs) in enumerate(latent_probs.items()):
            probs = probs.float()
            ax.boxplot([probs[:, i] for i in range(probs.shape[1])],
                      positions=positions + 0.3 * (idx + 1),
                      patch_artist=True,
                      boxprops=dict(facecolor=colors[idx + 1], alpha=0.5))
        
    elif method == "violin":
        # Violin plot showing full distribution
        data_list = [target_probs]
        labels = [target_lang]
        for label, probs in latent_probs.items():
            probs = probs.float()
            data_list.append(probs)
            labels.append(label)
            
        all_data = torch.stack(data_list)  # [n_distributions, n_examples, n_layers]
        for i in range(all_data.shape[2]):  # for each layer
            violin_parts = ax.violinplot([dist[:, i] for dist in all_data],
                                       positions=[i + j*0.3 for j in range(len(data_list))])
        ...
        # For target probs
        target_quants = torch.quantile(target_probs, torch.tensor(quantiles), dim=0)
        for i, q in enumerate(quantiles):
            ax.plot(target_quants[i], 
                   alpha=0.5 if i != 2 else 1.0,  # median darker
                   color=colors[0],
                   label=f"{target_lang} {q:.2f}" if i == 2 else None)
            
        # For latent probs
        for idx, (label, probs) in enumerate(latent_probs.items()):
            probs = probs.float()
            latent_quants = torch.quantile(probs, torch.tensor(quantiles), dim=0)
            for i, q in enumerate(quantiles):
                ax.plot(latent_quants[i],
                       alpha=0.5 if i != 2 else 1.0,
                       color=colors[idx + 1],
                       label=f"{label} {q:.2f}" if i == 2 else None)
                
    elif method == "heatmap":
        # Create heatmap showing distribution density across layers
        fig, axes = plt.subplots(1 + len(latent_probs), 1, 
                                figsize=(10, 3*(1 + len(latent_probs))),
                                sharex=True)
        
        # For target probs
        sns.heatmap(target_probs.T, ax=axes[0], cmap='viridis',
                    xticklabels=False, yticklabels=[f"L{i}" for i in range(target_probs.shape[1])])
        axes[0].set_title(target_lang)
        
        # For latent probs
        for idx, (label, probs) in enumerate(latent_probs.items()):
            probs = probs.float()
            sns.heatmap(probs.T, ax=axes[idx + 1], cmap='viridis',
                       xticklabels=False if idx != len(latent_probs)-1 else True,
                       yticklabels=[f"L{i}" for i in range(probs.shape[1])])
            axes[idx + 1].set_title(label)
    
    # Add baselines if provided
    if source_baseline is not None:
        ax.axhline(source_baseline, color='gray', linestyle='--', alpha=0.6)
    if target_baseline is not None:
        ax.axhline(target_baseline, color='gray', linestyle='-.', alpha=0.6)
