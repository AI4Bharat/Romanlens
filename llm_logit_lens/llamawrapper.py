...
class BlockOutputWrapper(torch.nn.Module):
    def __init__(self, block, unembed_matrix, norm):
        ...
        if hasattr(self.block, 'post_attention_layernorm'):
            ...
            # Handle the case where post_attention_layernorm doesn't exist
    ...
    def forward(self, *args, **kwargs):
        ...
        self.output_normalized = output[0]  # Remove the norm application
...
class LlamaHelper:
    def __init__(self, dir='/dlabdata1/llama2_hf/Llama-2-7b-hf', hf_token=None, device=None, load_in_8bit=True,load_in_4bit=False, use_embed_head=False, device_map='auto'):
        # if device is None:
        #     self.device = "cuda" if torch.cuda.is_available() else "cpu"
        # else:
        #     self.device = device
        if device_map == 'auto':
            # For single GPU, use cuda:0
    ...
    def latents_all_layers(self, text, return_attn_mech=False, return_intermediate_res=False, return_mlp=False, return_mlp_post_activation=False, return_block=True):
        ...
        if return_block:
            for i, layer in enumerate(self.model.model.layers):
                ...
                # Clear the GPU memory for this layer to save space
            # del layer.output
            # # torch.cuda.empty_cache()
        elif return_mlp_post_activation:
            for i, layer in enumerate(self.model.model.layers):
                tensors += [layer.mlp_post_activation.detach().cpu()]
                # del layer.mlp_post_activation
                # torch.cuda.empty_cache() 
        return torch.cat(tensors, dim=0)
    
    def latents_last_10_layers(self, text, return_attn_mech=False, return_intermediate_res=False, return_mlp=False, return_mlp_post_activation=False, return_block=True):
        if return_attn_mech or return_intermediate_res or return_mlp:
            raise NotImplemented("not implemented")
        device = next(self.model.parameters()).device 
        self.get_logits(text)
        ...
        if return_block:
            ...
            start_layer = max(0, total_layers - 10)  # Get starting index for last 10 layers
